...
![Java程序运行过程](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/Java%20%E7%A8%8B%E5%BA%8F%E8%BF%90%E8%A1%8C%E8%BF%87%E7%A8%8B.png)
...
![](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/20210425151835918.png)
...
1. **形式** : 字符常量是单引号引起的一符，字符串常量是双引号引起的 0 个或若干个字符

2. **含义** : 字符常量相当于一个整型值( ASCII 值),可以参加表达式运算; 字符串常量代表一个地址值(该字符串在内存中存放位

3. **占内存大小** ： 字符Char常量只占 2 个字节; 字符串String常量占若干个字节 (**注意： char 在 Java 中占两个字节**),

   > 字符封装类 `Character` 有一个成员常量 `Character.SIZE` 值为 16,单位是`bits`,该值除以 8(`1byte=8bits`)后就可以得到 2 个字节

> java 编程思想第四版：2.2.2 节
> ![](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/86735519.jpg)
...
hashcode会把你创建的对象中的内容通过一定的算法计算得出hash值，源代码如下

```java
		// 你重写的hashcode方法
		@Override
    public int hashCode() {
        return Objects.hash(你传入的成员变量);
    }
...
不难看出，如果对象中的内容相同，则生成的hash码肯定相同，但内容相同并不意味着是同一个对象



**注：**以上只有在你重写hashcode的前提下成立，如果没有重写hashcode，则直接使用native中的hashcode方法，new两个相同对象的hashcode就不同了
...
包装类型不赋值就是 `Null` ，而基本数据类型必须要有默认值且不是 `Null`。
...
![](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/20210422164544846.png)
...
![example 1 ](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/22191348.jpg)

在 swap 方法中，a、b 的值进行交换，并不会影响到 num1、num2。因为，a、b 中的值，只是从 num1、num2 的复制过来的。也就是说，a、b 相当于 num1、num2 的副本，副本的内容无论怎么修改，都不会影响到原件本身。

**通过上面例子，我们已经知道了一个方法不能修改一个基本数据类型的参数，而对象引用作为参数就不一样，请看 example3**
...
![example 2](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/3825204.jpg)
...
![](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/88729818.jpg)

交换之后：

![](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/34384414.jpg)
...
![](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/desktopjava%E6%A0%B8%E5%BF%83%E6%8A%80%E6%9C%AF-%E9%87%8D%E8%BD%BD.jpg)

综上：重载就是同一个类中多个同名方法根据不同的传参来执行不同的逻辑处理。

#### 重写

重写发生在运行期，是指子类对父类的允许访问的方法的实现过程进行重新编写。
...
- “两同”即方法名相同、形参列表相同；
- “两小”指的是子类方法返回值类型应 <=父类方法返回值类型，子类方法声明抛出的异常类应 <= 父类方法声明抛出的异常类
...
1. **浅拷贝**：对基本数据类型进行值传递，对引用数据类型进行引用传递般的拷贝，此为浅拷贝(拷贝出来的另一个的对象内容相同地址值指向同一个)。
2. **深拷贝**：对基本数据类型进行值传递，对引用数据类型，创建一个新的对象，并复制其内容，此为深拷贝（不但内容相同地址值也拷贝出了新的）。

![deep and shallow copy](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/java-deep-and-shallow-copy.jpg)
...
4. 从变量是否有默认值来看，成员变量如果没有被赋值，则会自动以类型的默认值而赋值（一种情况例外:被 `final` 修饰的成员变量也必须显式地赋值），而局部变量则不会自动赋值。
...
![](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/Java%E5%BC%82%E5%B8%B8%E7%B1%BB%E5%B1%82%E6%AC%A1%E7%BB%93%E6%9E%84%E5%9B%BE.png)

<p style="font-size:13px;text-align:right">图片来自：https://simplesnippets.tech/exception-handling-in-java-part-1/</p>

![](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/Java%E5%BC%82%E5%B8%B8%E7%B1%BB%E5%B1%82%E6%AC%A1%E7%BB%93%E6%9E%84%E5%9B%BE2.png)
...
![check-exception](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/check-exception.png)
...
![](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/a478c74d-2c48-40ae-9374-87aacf05188c.png)
...
![IO-操作方式分类](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/IO-%E6%93%8D%E4%BD%9C%E6%96%B9%E5%BC%8F%E5%88%86%E7%B1%BB.png)

按操作对象分类结构图：

![IO-操作对象分类](https://figure-bed-liqun.oss-cn-beijing.aliyuncs.com/uPic/IO-%E6%93%8D%E4%BD%9C%E5%AF%B9%E8%B1%A1%E5%88%86%E7%B1%BB.png)
